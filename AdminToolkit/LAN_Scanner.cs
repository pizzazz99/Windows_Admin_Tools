...
namespace Admin_Tools
{
    ...
    internal sealed class Rdp_Target
    {
        ...
        public string ConnectTarget =>
            !string.IsNullOrEmpty(Ip) ? Ip : Name;
    ...
    internal static class Lan_Scanner
    {
        ...
        public static async Task<List<Rdp_Target>> Get_Hosts(bool forceRescan = false)
        {
            ...
            {
                bool known = targets.Any(t =>
                    string.Equals(t.Name, name, StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(t.ConnectTarget, name, StringComparison.OrdinalIgnoreCase));
        ...
        private static List<string> Browse_List_Names()
        {
            ...
            try
            {
                var psi = new ProcessStartInfo("net", "view")
                {
                    RedirectStandardOutput = true,
                    UseShellExecute = false,
                    CreateNoWindow = true
        ...
        {
            _ = t.ContinueWith(x => { var _ignored = x.Exception; },
        ...
        private static IEnumerable<string> Local_Subnets()
        {
            ...
            foreach (NetworkInterface ni in NetworkInterface.GetAllNetworkInterfaces())
            {
                ...
                foreach (UnicastIPAddressInformation ua in ni.GetIPProperties().UnicastAddresses)
                {
                    ...
                    if (Is_Tailscale_Cgnat(ua.Address)) continue; // 100.64/10

                    int last = ip.LastIndexOf('.');
                    if (last > 0) prefixes.Add(ip.Substring(0, last));
                }
            }
            return prefixes;
        }
