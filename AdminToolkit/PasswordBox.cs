  ...
  internal static class PasswordBox
  {
    ...
    public static string Show( IWin32Window owner, string title, string prompt )
    {
      using ( var form = new Form() )
      {
        ...
        var lbl = new Label { Text = prompt, AutoSize = false, Bounds = new Rectangle( 12, 12, 376, 50 ) };

        var txt = new TextBox { Bounds = new Rectangle( 12, 68, 376, 25 ), UseSystemPasswordChar = true };

        var ok = new Button { Text = "OK", DialogResult = DialogResult.OK, Bounds = new Rectangle( 224, 100, 80, 28 ) };

        var cancel = new Button { Text = "Cancel", DialogResult = DialogResult.Cancel, Bounds = new Rectangle( 308, 100, 80, 28 ) };
