...
namespace Admin_Tools
{
    ...
    internal static class Emailer
    {
        ...
        public class SmtpConfig
        {
            ...
            {
                get { return ServerReady && !string.IsNullOrEmpty(Password); }
            }
        }
                ...
                else
                {
                    foreach (string raw in File.ReadAllLines(_settingsPath))
                    {
                        ...
                        switch (key)
                        {
                            ...
                            case "port":      int.TryParse(val, out cfg.Port); break;
                            case "enablessl": bool.TryParse(val, out cfg.EnableSsl); break;
                            case "user":      cfg.User = val; break;
                            case "from":      cfg.From = val; break;
                            case "to":        cfg.To = val; break;
                            // NOTE: no "password" here - it lives in the encrypted store.
                        }
                    }
                }

                if (string.IsNullOrEmpty(cfg.From)) cfg.From = cfg.User;
                cfg.Password = LoadPassword();
            }
            catch { /* return whatever we managed to parse */ }
            return cfg;
        }
        ...
        public static void OpenSettings()
        {
            try
            {
                if (!File.Exists(_settingsPath)) WriteTemplate();
        ...
        {
            try { if (File.Exists(_secretPath)) File.Delete(_secretPath); }
            catch { }
        }
        ...
        public static void Send(string subject, string body, string attachmentPath = null)
        {
            ...
            if (!cfg.IsComplete)
            {
                throw new InvalidOperationException(
                    "Email is not fully configured (server details and/or password missing).");
            }
