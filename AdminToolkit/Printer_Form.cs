...
namespace Admin_Tools
{
    public partial class Printer_Form : Form
    {
        ...
        //  Printer list
        // --------------------------------------------------------
        private void Load_Printers()
        {
            Cursor = Cursors.WaitCursor;
            try
            {
                var all = Printer_Support.Get_Installed_Printers();
                var printers = chkHideVirtual.Checked
                    ? all.Where(p => !p.IsVirtual).ToList()
                    : all;
                ...
                foreach (var p in printers)
                {
                    ...
                    item.SubItems.Add(p.IsVirtual ? "Virtual"
        ...
        private async void Refresh_Live_Status()
        {
            ...
            {
                if (items[i].ListView == null) continue;   // list was reloaded meanwhile
        ...
        private async void Btn_Wake_Click(object Sender, EventArgs e)
        {
            ...
            if (p == null)
            {
                MessageBox.Show("Select a printer first.", "Wake / Retry",
            ...
            try
            {
                ...
                else
                {
                    ...
                    liveCell.ForeColor = Color.Firebrick;
                    MessageBox.Show(
                        p.Name + " did not respond after repeated attempts (~9 seconds). " +
                        "It may be fully powered off, on a different subnet/VLAN, or blocking " +
        ...
        {
            if (lvPrinters.SelectedItems.Count == 0) return null;
        ...
        private void Btn_Details_Click(object Sender, EventArgs e)
        {
            ...
            if (p == null)
            {
                MessageBox.Show("Select a printer first.", "Printer Details",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private async void Btn_Supplies_Click(object Sender, EventArgs e)
        {
            ...
            if (p == null)
            {
                MessageBox.Show("Select a printer first.", "Ink / Toner Levels",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            try
            {
                ...
                if (!result.Success)
                {
                    MessageBox.Show(result.Error, "Ink / Toner Levels",
                ...
                foreach (var s in result.Supplies)
                {
                    ...
                    sb.AppendLine("    " +
                        (s.Percent.HasValue ? s.Percent.Value + "%" : (s.RawNote ?? "Unknown")));
                    sb.AppendLine();
                }
