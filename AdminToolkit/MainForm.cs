...
namespace Admin_Tools
{
  public partial class MainForm : Form
  {
    ...
    // Internal — Admin_Tools_Form colors its own launcher buttons on load
    // with this same palette, so the two windows stay visually consistent.
    internal static readonly System.Drawing.Color _ReadyColor =
    ...
    private Admin_Tools_Form _AdminToolsForm;
    ...
    private void MainForm_Load( object Sender, EventArgs E )
    {
      ...
      // Admin Tools launcher buttons now live on Admin_Tools_Form, which
      // colors them green (ready) itself on construction.
    ...
    private void CaptureControlBounds( Control Parent )
    {
      ...
      foreach (Control C in Parent.Controls)
      {
        ...
        if (C is System.Windows.Forms.ListView Lv)
        {
    ...
    private void LaunchTool( string Display_Name, string File_Name, string Arguments,
                              System.Windows.Forms.Button Launcher, string Window_Title = null )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      try
      {
    ...
    internal void BtnTaskScheduler_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Task Scheduler", File_Name: "mmc.exe",
                   Arguments: Sys32( "taskschd.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnSystemProtection_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "System Protection",
                   File_Name: Sys32( "SystemPropertiesProtection.exe" ), Arguments: "",
                   Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnRestoreWizard_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "System Restore Wizard", File_Name: Sys32( "rstrui.exe" ),
                   Arguments: "", Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnRegistryEditor_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
    ...
    internal void BtnEventViewer_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Event Viewer", File_Name: "mmc.exe",
                   Arguments: Sys32( "eventvwr.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnServices_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Services", File_Name: "mmc.exe",
                   Arguments: Sys32( "services.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnDiskManagement_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Disk Management", File_Name: "mmc.exe",
                   Arguments: Sys32( "diskmgmt.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnComputerMgmt_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Computer Management", File_Name: "mmc.exe",
                   Arguments: Sys32( "compmgmt.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnSystemInfo_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
    ...
    internal void BtnPerfMonitor_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Performance Monitor", File_Name: "mmc.exe",
                   Arguments: Sys32( "perfmon.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnResourceMonitor_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Resource Monitor", File_Name: Sys32( "resmon.exe" ),
                   Arguments: "", Launcher: (System.Windows.Forms.Button) Sender, Window_Title: "Resource Monitor" );
    }

    internal void BtnDeviceManager_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Device Manager", File_Name: "mmc.exe",
                   Arguments: Sys32( "devmgmt.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnLocalUsers_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Local Users && Groups", File_Name: "mmc.exe",
                   Arguments: Sys32( "lusrmgr.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnFirewall_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Windows Firewall", File_Name: "mmc.exe",
                   Arguments: Sys32( "wf.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }


    internal void Task_Manager_Button_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Task Manager", File_Name: Sys32( "taskmgr.exe" ),
                   Arguments: null, Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnLocalSecurityPolicy_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Local Security Policy", File_Name: "mmc.exe",
                   Arguments: Sys32( "secpol.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnCertManager_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Certificate Manager (Local Machine)", File_Name: "mmc.exe",
                   Arguments: Sys32( "certlm.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnWmiControl_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "WMI Control", File_Name: "mmc.exe",
                   Arguments: Sys32( "wmimgmt.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnSharedFolders_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Shared Folders", File_Name: "mmc.exe",
                   Arguments: Sys32( "fsmgmt.msc" ), Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnAdvancedSystemProps_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Advanced System Properties",
                   File_Name: Sys32( "SystemPropertiesAdvanced.exe" ), Arguments: "",
                   Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnPerformanceOptions_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Performance Options",
                   File_Name: Sys32( "SystemPropertiesPerformance.exe" ), Arguments: "",
                   Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnRemoteSettings_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Remote Settings",
                   File_Name: Sys32( "SystemPropertiesRemote.exe" ), Arguments: "",
                   Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnReliabilityMonitor_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Reliability Monitor", File_Name: Sys32( "perfmon.exe" ),
                   Arguments: "/rel", Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnWindowsFeatures_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Windows Features", File_Name: Sys32( "OptionalFeatures.exe" ),
                   Arguments: "", Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnProgramsFeatures_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "Programs and Features", File_Name: Sys32( "control.exe" ),
                   Arguments: "appwiz.cpl", Launcher: (System.Windows.Forms.Button) Sender );
    }

    internal void BtnSystemConfig_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      ToggleTool( Display_Name: "System Configuration", File_Name: Sys32( "msconfig.exe" ),
                   Arguments: "", Launcher: (System.Windows.Forms.Button) Sender );
    }
    ...
    private void Admin_Tools_Button_Click( object Sender, EventArgs E )
    {
      using var Block = Trace_Block.Start_If_Enabled();
      if (_AdminToolsForm == null || _AdminToolsForm.IsDisposed)
      {
        _AdminToolsForm = new Admin_Tools_Form( this );
        _AdminToolsForm.FormClosed += ( S, Args ) => Admin_Tools_Button.Enabled = true;
        _AdminToolsForm.Show( this );
        Admin_Tools_Button.Enabled = false;
      }
      else
      {
        if (_AdminToolsForm.WindowState == FormWindowState.Minimized)
          _AdminToolsForm.WindowState = FormWindowState.Normal;
        _AdminToolsForm.BringToFront();
      }
    }
