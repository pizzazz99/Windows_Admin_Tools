...
namespace Admin_Tools
{
    ...
    internal static class Logger
    {
        ...
        public static string CurrentLogPath
        {
            get
            {
                string name = string.Format("log-{0}-{1}.txt",
            ...
            try
            {
                lock (_gate)
                {
                    ...
                    string line = string.Format(
                        "{0:yyyy-MM-dd HH:mm:ss}  [{1,-9}]  {2}  ({3}\\{4})",
                        DateTime.Now,
                        category,
                        message,
            ...
            try
            {
                lock (_gate)
                {
                    ...
                    sb.AppendLine(string.Format(
                        "SESSION START  {0:yyyy-MM-dd HH:mm:ss}  {1}  ({2}\\{3})  elevated={4}",
                        DateTime.Now, Environment.MachineName,
                        Environment.UserDomainName, Environment.UserName, elevated));
                    sb.AppendLine(new string('=', 70));
