  ...
  public partial class MainForm : Form
  {
    ...
    private static bool IsShadowingActive( string drive )
    {
      try
      {
        ...
        using ( var key = Registry.LocalMachine.OpenSubKey( @"SOFTWARE\Microsoft\Windows NT\CurrentVersion\SPP\Clients" ) )
        {
          ...
          return data != null && data.Any( v => v.IndexOf( volumeGuid, StringComparison.OrdinalIgnoreCase ) >= 0 );
        }
      }
      catch
      {
    ...
    private static List<string> GetFixedNtfsDrives()
    {
      return DriveInfo.GetDrives()
        .Where( d => d.DriveType == DriveType.Fixed && d.IsReady && string.Equals( d.DriveFormat, "NTFS", StringComparison.OrdinalIgnoreCase ) )
        .Select( d => d.Name )
        .ToList();
    }
    ...
    private List<DriveChoice> BuildDriveChoices()
    {
      ...
      foreach ( var d in DriveInfo.GetDrives() )
      {
        if ( d.DriveType != DriveType.Fixed || ! d.IsReady )
          continue;
        if ( ! string.Equals( d.DriveFormat, "NTFS", StringComparison.OrdinalIgnoreCase ) )
          continue;
        ...
        choices.Add( new DriveChoice { Name    = d.Name,
                                       Display = string.Format( "{0}  {1}  ({2:0} GB){3}", d.Name.TrimEnd( '\\' ), label, gb, active ? "   — already active" : "" ),
    ...
    private bool RegisterShadowTask( IReadOnlyList<string> drives )
    {
      ...
      string       psInner    = "foreach($v in " + volumeList + "){Invoke-CimMethod -ClassName Win32_ShadowCopy " +
                                "-Namespace root/cimv2 -MethodName Create -Arguments @{Volume=$v;Context='ClientAccessible'}}";
      string       psArgs     = System.Security.SecurityElement.Escape( "-NoProfile -WindowStyle Hidden -Command \"" + psInner + "\"" );

      string       xml = "<?xml version=\"1.0\" encoding=\"UTF-16\"?>" + "<Task version=\"1.2\" xmlns=\"http://schemas.microsoft.com/windows/2004/02/mit/task\">" +
                         "<Triggers><CalendarTrigger><StartBoundary>2020-01-01T12:00:00</StartBoundary>" + "<Enabled>true</Enabled><ScheduleByDay><DaysInterval>1</DaysInterval></ScheduleByDay>" +
                         "</CalendarTrigger></Triggers>" + "<Principals><Principal id=\"Author\"><UserId>S-1-5-18</UserId>" + "<RunLevel>HighestAvailable</RunLevel></Principal></Principals>" +
                         "<Settings><MultipleInstancesPolicy>IgnoreNew</MultipleInstancesPolicy>" + "<StartWhenAvailable>true</StartWhenAvailable></Settings>" + "<Actions Context=\"Author\"><Exec>" +
                         "<Command>powershell.exe</Command><Arguments>" + psArgs + "</Arguments>" + "</Exec></Actions>" + "</Task>";
      ...
      var t = RunCapture( "schtasks.exe", "/Create /TN \"" + taskName + "\" /XML \"" + xmlPath + "\" /F" );
    ...
    private bool CreateShadowTask( IReadOnlyList<string> selected, out string error )
    {
      ...
      var    r         = RunCapture( "powershell.exe", "-NoProfile -ExecutionPolicy Bypass -Command " + "\"$ErrorActionPreference='Stop'; Enable-ComputerRestore -Drive " + driveArgs + "\"" );
      ...
      {
        Logger.Log( "Shadow", "Throttle registry skipped: " + Ex.Message );
      }
      ...
      bool newlyEnabled = protectedNow.Any( d => ! beforeActive.Any( b => b.Equals( d, StringComparison.OrdinalIgnoreCase ) ) );
    ...
    private void BtnSetupShadowing_Click(object Sender, EventArgs E )
    {
      ...
      {
        statusLabel.Text = "Shadowing enabled on " + string.Join( ", ", drives ) + " and daily task created (ran once now).";
        // RefreshSnapshots();
      }
      else
      {
        MessageBox.Show( "Setup failed.\n\n" + error + "\n\nCheck that you're elevated and that System " + "Restore isn't disabled by group policy. See the log for the full details.",
                         "Setup Failed",
    ...
    private bool TeardownShadowing( IReadOnlyList<string> offDrives, IReadOnlyList<string> survivors, out string error )
    {
      const string taskName = "AutoShadowCopy";
      error                 = "";
      ...
      var    r         = RunCapture( "powershell.exe", "-NoProfile -ExecutionPolicy Bypass -Command " + "\"$ErrorActionPreference='Stop'; Disable-ComputerRestore -Drive " + driveArgs + "\"" );
      ...
      else
      {
        ...
        try
        {
          using ( var key = Registry.LocalMachine.OpenSubKey( @"SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRestore", writable: true ) )
          {
        ...
        {
          Logger.Log( "Shadow", "Throttle cleanup skipped: " + Ex.Message );
        }
      }
    ...
    private void BtnTeardownShadowing_Click(object Sender, EventArgs E )
    {
      ...
      var    protectedDrives = GetFixedNtfsDrives().Where( name => IsShadowingActive( name ) ).ToList();
      ...
      var          choices = protectedDrives
                               .Select( name =>
                                 ...
                                 return new DriveChoice { Name    = name,
                                                          Display = string.Format( "{0}  {1}  ({2:0} GB){3}", name.TrimEnd( '\\' ), label, gb, isSys ? "   — system drive" : "" ),
                                                          Checked = false };
                                        } )
                               .ToList();

      List<string> offList;
      using ( var dlg = new DriveSelectDialog( choices, "Turn Off Shadowing", "Check the drives you want to TURN OFF (this deletes their snapshots):" ) )
      {
        if ( dlg.ShowDialog( this ) != DialogResult.OK )
        {
      ...
      string taskFate = survivors.Count > 0 ? "The AutoShadowCopy task will be rebuilt to snapshot only: " + string.Join( ", ", survivors ) + "." : "The AutoShadowCopy task will be removed entirely.";
      string cascadeNote = cascaded ? "\n\nNote: turning off the system drive (" + sysDrive.TrimEnd( '\\' ) + ") also turns off the other protected drives — Windows requires this." : "";

      var    confirm = MessageBox.Show( "This will permanently delete all snapshots on: " + willDelete + "." + cascadeNote + "\n\n" + taskFate + "\n\nSnapshots cannot be recovered. Continue?",
      ...
      {
        statusLabel.Text = "Shadowing turned off on " + willDelete + ( survivors.Count > 0 ? " — task now covers " + string.Join( ", ", survivors ) + "." : " — task removed." );
        // RefreshSnapshots();
      }
      else
      {
        MessageBox.Show( "Turn-off failed.\n\n" + error + "\n\nCheck the log for details.", "Teardown Failed", MessageBoxButtons.OK, MessageBoxIcon.Warning );
